...
impl From<CoreTokenUsage> for TokenUsageBreakdown {
    fn from(value: CoreTokenUsage) -> Self {
        let to_i64 = |count| i64::try_from(count).unwrap_or(i64::MAX);
        Self {
            total_tokens: to_i64(value.total_tokens),
            input_tokens: to_i64(value.input_tokens),
            cached_input_tokens: to_i64(value.cached_input_tokens),
            output_tokens: to_i64(value.output_tokens),
            reasoning_output_tokens: to_i64(value.reasoning_output_tokens),
        }
    }
}
