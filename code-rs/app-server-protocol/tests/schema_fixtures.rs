...
fn schema_root() -> Result<std::path::PathBuf> {
    ...
    let typescript_index = code_utils_cargo_bin::find_resource!("schema/typescript/index.ts")
    ...
    let json_bundle =
        code_utils_cargo_bin::find_resource!("schema/json/code_app_server_protocol.schemas.json")
            .context("resolve JSON schema bundle")?;
    let json_root = json_bundle
        .parent()
        .and_then(|p| p.parent())
        .context("derive schema root from schema/json/code_app_server_protocol.schemas.json")?;
    anyhow::ensure!(
