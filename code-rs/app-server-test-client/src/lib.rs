...
use code_app_server_protocol::CommandExecutionRequestApprovalParams;
use code_app_server_protocol::CommandExecutionRequestApprovalResponse;
use code_app_server_protocol::DynamicToolSpec;
use code_app_server_protocol::FileChangeApprovalDecision;
use code_app_server_protocol::FileChangeRequestApprovalParams;
...
use code_app_server_protocol::InputItem;
...
use code_app_server_protocol::LoginChatGptCompleteNotification;
...
struct Cli {
    /// Path to the `code` CLI binary.
    #[arg(long, env = "CODE_BIN", default_value = "code")]
    codex_bin: PathBuf,

    /// Forwarded to the `code` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
impl CodexClient {
    fn spawn(codex_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let stdin = codex_app_server
            .stdin
            .take()
            .context("code app-server stdin unavailable")?;
        let stdout = codex_app_server
            .stdout
            .take()
            .context("code app-server stdout unavailable")?;
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "code-toy-app-server".to_string(),
                    title: Some("Code Toy App Server".to_string()),
    ...
    fn remove_thread_listener(&mut self, subscription_id: Uuid) -> Result<()> {
        ...
        let request = ClientRequest::RemoveConversationListener {
            request_id: request_id.clone(),
            params: code_app_server_protocol::RemoveConversationListenerParams { subscription_id },
        };

        self.send_request::<code_app_server_protocol::RemoveConversationSubscriptionResponse>(
            request,
            request_id,
    ...
    fn write_request(&mut self, request: &ClientRequest) -> Result<()> {
        ...
        if let Some(stdin) = self.stdin.as_mut() {
            ...
            stdin
                .flush()
                .context("failed to flush request to code app-server")?;
        } else {
            bail!("code app-server stdin closed");
        }
    ...
    fn read_jsonrpc_message(&mut self) -> Result<JSONRPCMessage> {
        loop {
            ...
            let bytes = self
                ...
                .context("failed to read from code app-server")?;

            if bytes == 0 {
                bail!("code app-server closed stdout");
            }
    ...
    fn write_jsonrpc_message(&mut self, message: JSONRPCMessage) -> Result<()> {
        ...
        if let Some(stdin) = self.stdin.as_mut() {
            ...
            stdin
                .flush()
                .context("failed to flush response to code app-server")?;
            return Ok(());
        }
...
impl Drop for CodexClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = self.child.try_wait() {
            println!("[code app-server exited: {status}]");
            return;
        }
