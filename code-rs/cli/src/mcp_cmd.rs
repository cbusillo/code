...
/// - `add`    — add a server launcher entry to `~/.code/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "code mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `code mcp login {name}` to login."
        ),
    }
