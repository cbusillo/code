...
"#;
    ...
    fn upgrade_legacy_model_slugs_updates_top_level() {
        ...
        let mut gemini_cfg = ConfigToml {
            model: Some("gemini-3-pro".to_string()),
            review_model: Some("gemini-2.5-pro".to_string()),
            ..Default::default()
        };

        upgrade_legacy_model_slugs(&mut gemini_cfg);

        assert_eq!(gemini_cfg.model.as_deref(), Some("gemini-3.1-pro-preview"));
        assert_eq!(
            gemini_cfg.review_model.as_deref(),
