...
mod tests {
    ...
    fn supports_jq_with_directory_path() {
        assert_parsed(
            &vec_str(&["jq", "-r", ".name, .bin", "sdk/typescript/package.json"]),
            vec![ParsedCommand::Search {
                cmd: "jq -r '.name, .bin' sdk/typescript/package.json".to_string(),
                query: Some(".name, .bin".to_string()),
                path: Some("sdk/typescript/package.json".to_string()),
            }],
        );
    }
