...
fn python_search_dirs(cwd: &Path, files: &[PathBuf]) -> Vec<PathBuf> {
    ...
    for relative in files {
        let Some(mut current) = cwd.join(relative).parent().map(Path::to_path_buf) else {
            continue;
        };
        loop {
...
mod tests {
    ...
    fn resolves_nearest_virtualenv_tool_before_path() {
        ...
        let resolved = find_virtualenv_tool(
            cwd,
            &[Path::new("packages/app/src/main.py").to_path_buf()],
            "mypy",
        )
        .expect("resolve local mypy");

        assert_eq!(resolved.executable, nested_bin.join("mypy"));
        assert!(resolved.env_updates.iter().any(|(key, value)| {
            key == "VIRTUAL_ENV" && value == nested_root.join(".venv").as_os_str()
        }));
    }
    ...
    fn markdownlint_runs_from_repo_root_so_repo_config_is_visible() {
        ...
        let path_guard = ScopedEnvVar::set("PATH", Some(bin_dir.into_os_string()));
        assert!(
            which(Path::new("markdownlint")).is_some(),
            "fake markdownlint not found on PATH"
        );
        let action = ApplyPatchAction::new_add_for_test(
