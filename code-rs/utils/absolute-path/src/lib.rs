...
mod tests {
    ...
    fn guard_used_in_deserialization() {
        ...
        let json = serde_json::to_string(relative_path).expect("serialize");

        let _guard = AbsolutePathBufGuard::new(base_dir);
        let absolute_path: AbsolutePathBuf = serde_json::from_str(&json).expect("deserialize");
        assert_eq!(absolute_path.as_path(), base_dir.join(relative_path).as_path());
    }
}
