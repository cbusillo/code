...
| --- | --- | --- | --- | --- | --- |
| Auto Drive core and TUI | `code-rs/code-auto-drive-core/`, `code-rs/code-auto-drive-diagnostics/`, `code-rs/core/src/auto_drive_pid.rs`, `code-rs/tui/src/auto_drive_*`, `code-rs/tui/src/chatwidget/auto_drive_cards.rs`, `docs/auto-drive.md` | Product docs preserve the Auto Drive intent, but current `main` does not expose the old coordinator, `/auto`, `code exec --auto`, Auto Drive cards, or settings pane. The #398 boundary map lives in [auto-drive-parity.md](auto-drive-parity.md). | Rewrite on Codex primitives. Preserve the user promises through fixtures before rebuilding a coordinator. | High | Start with continue-mode, goal, review-gate, duplicate-ownership, and long-session rendering fixtures on current thread/session/goal/review surfaces. |
