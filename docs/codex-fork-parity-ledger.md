...
| --- | --- | --- | --- | --- |
| Auto Drive coordinator, state, and TUI | `code-rs/code-auto-drive-core/`, `code-rs/code-auto-drive-diagnostics/`, `code-rs/core/src/auto_drive_pid.rs`, `code-rs/tui/src/auto_drive_strings.rs`, `code-rs/tui/src/auto_drive_style.rs`, `code-rs/tui/src/bottom_pane/auto_coordinator_view.rs`, `code-rs/tui/src/bottom_pane/auto_drive_settings_view.rs`, `code-rs/tui/src/chatwidget/auto_drive_cards.rs`, `code-rs/tui/tests/auto_drive_long_session_perf.rs`, and `vt100_chatwidget_snapshot__auto_drive_*.snap` | Docs still describe Auto Drive, but the dedicated crates, coordinator UI, TUI cards, and snapshots are absent from current `main`. Codex-base has thread/session/goal/token surfaces that should host any replacement. | Port | #398 owns Auto Drive parity fixtures. First restore or rewrite controller/coordinator regression fixtures, then rebuild on Codex-base thread/session primitives. |
| Code Bridge and browser control | `code-rs/browser/`, `code-rs/browser/tests/local_navigation.rs`, `code-rs/core/src/bridge_client.rs`, `code-rs/tui/src/chatwidget/browser_sessions.rs`, `code-rs/tui/src/history_cell/browser.rs`, `vt100_chatwidget_snapshot__browser_session_*.snap`, `vt100_chatwidget_snapshot__context_cell_browser_badge.snap` | The dedicated browser crate and Code Bridge client are absent. Current app-server/Desktop compatibility has remote-control shims, but not the Every Code local bridge/control channel. | Port | #399 owns the bridge/browser parity boundary. Preserve console/error/pageview/screenshot/control semantics as an additive Every Code overlay. |
...
| `code-rs/tui/tests/auto_drive_long_session_perf.rs` and `vt100_chatwidget_snapshot__auto_drive_*.snap` | Port | #398 | Captured Auto Drive coordinator, countdown, review, and long-session behavior. |
